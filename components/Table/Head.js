...
var DISPLAY_NAME = 'C4tTableHead';
export var Head = forwardRef(function(_a, ref) {
  var _b;
  ...
  useEffect(function() {
    if (!Array.isArray(children)) {
      validateChildComponent('Head', children, 'TableRow', Row);
      return;
    }
    if (children.length === 0) {
      if (process.env.NODE_ENV !== 'production') {
        console.error('Head must have at least one Row as child');
      }
    }
    for (var _i = 0, children_1 = children; _i < children_1.length; _i++) {
      var node = children_1[_i];
      validateChildComponent('Head', node, 'TableRow', Row);
    }
  }, [children]);
